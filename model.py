...
class Model:
    def __init__(self, model_path: str = None, adapter_path: str = None):
        self.model_path = model_path or os.getenv("MODEL_PATH", "Qwen/Qwen3-8B")
        ...
        try:
            ...
            if self.device == "cuda":
                ...
                self.model = AutoModelForCausalLM.from_pretrained(
                    ...
                    device_map={"": "cuda:0"},  # GPU 전용 로딩 (오프로딩 방지)
                    trust_remote_code=True,
                    torch_dtype=getattr(torch, self.torch_dtype),
                    attn_implementation="sdpa"  # Flash Attention 2 대신 SDPA 사용
                )
            else:
                # CPU 모드
                self.model = AutoModelForCausalLM.from_pretrained(
